...
class ImageMaskDatasetGenerator:
  def __init__(self, size=512, exclude_empty_mask=True, augmentation=True):
  ...
  def colorize_mask(self, mask):
    ...
    colorized[np.greater(mask, 128)] = bgr_color
    return colorized


  def generate(self, patients_ct_dir, output_images_dir, output_masks_dir):

    self.output_images_dir = output_images_dir
    self.output_masks_dir  = output_masks_dir
    
    mask_files = sorted(glob.glob(patients_ct_dir + "/*/brain/*_HGE_Seg.jpg"))
    ...
    for i in range(num):
      ...
      image_file = mask_file.replace("_HGE_Seg", "")
      self.generate_mask_file(mask_file,   index, output_masks_dir)
      self.generate_image_file(image_file, index, output_images_dir)

  # Generate sliced images by combining ct_slices and pet_slices 
  def generate_image_file(self, image_file, index, output_images_dir):
  ...
  def generate_mask_file(self, mask_file, index, output_masks_dir):
...
if __name__ == "__main__":
  try:
    # Intracranial Hemorrhage Masks
    patients_ct_dir = "./Brain-CT/Patients_CT/"
    
    output_dir  = "./Brain-Intracranial-Hemorrhage"
    ...
    generator = ImageMaskDatasetGenerator(size=650,  augmentation=True)
    generator.generate(patients_ct_dir, output_images_dir, output_masks_dir)
